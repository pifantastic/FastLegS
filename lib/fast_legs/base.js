      ...
      ['many', 'belongsTo', 'one'].forEach(function(relationship) {
        ...
        finders[includeFinder] = function(cb) {
          ...
          if (relationship === 'many') {
            ...
            if (includedModel.through) {
              includes[includeFinder].join = includedModel.through;
              where[includedModel.through[0] + '.' + includedModel.joinOn] = model[self.primaryKey];
            }
            else if (includedModel.joinOn.split('.').length > 1) {
